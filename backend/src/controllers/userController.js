...
export const bookmarkArticle = async (req, res) => {
  try {
    ...
    const alreadyBookmarked = user.bookmarkedArticles.includes(articleId);
...
export const checkBookmarkStatus = async (req, res) => {
  try {
    const user = await User.findById(req.user._id);
    const { articleId } = req.params;
    const bookmarked = user.bookmarkedArticles.some(
      (id) => id.toString() === articleId
    );
    res.json({ bookmarked });
  } catch (error) {
  ...
  try {
    ...
    const readingHistory = [
      ...new Map(
        user.readingHistory
          .slice(-20)
          .reverse()
          .map((item) => [item.article._id.toString(), item.article])
      ).values(),
    ].slice(0, 10);
    ...
    res.json({
      ...
      stats: {
        ...
        favoriteCategory,
        digestSubscribed: user.digestSubscribed,
      },
      bookmarks,
      readingHistory,
      recommendations,
...
// SUBSCRIBE TO DIGEST
export const subscribeDigest = async (req, res) => {
  try {
    req.user.digestSubscribed = true;
    await req.user.save();
    res.json({ message: "Subscribed to weekly digest" });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
...
export const unsubscribeDigest = async (req, res) => {
  try {
    req.user.digestSubscribed = false;
    await req.user.save();
    res.json({ message: "Unsubscribed from weekly digest" });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};